...
export default function DashboardPage({ onLoggedOut }) {
  ...
  const load = useCallback(() => {
    getDashboardOverview({ report_date: stageReportDate })
      ...
      .catch((err) => {
        ...
        else setError("Dashboard লোড করা যায়নি।");
      });
  ...
  if (!data) return <div className="mx-auto max-w-3xl px-5 py-14 text-center text-sm text-[#667085]">লোড হচ্ছে…</div>;
    ...
    <div className="w-full max-w-none px-2 py-3">
      <div className="mb-6">
        ...
        <p className="text-xs text-[#667085]">
          গতকাল ({data.yesterday_date}) ও আজকের ({data.today_date}) কার্যক্রমের সারসংক্ষেপ।
        </p>
        </div>
      </div>

      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard label="গতকাল নতুন Registration" value={data.totals.registrations_yesterday} accent />
        <StatCard label="আজ নতুন Registration" value={data.totals.registrations_today} />
      ...
      <div className="mb-6 grid gap-4 lg:grid-cols-2">
        <LeadBreakdownCard title="আজকের নতুন Lead" data={data.today_leads} />
        <LeadBreakdownCard title="আগের দিনের নতুন Lead" data={data.yesterday_leads} />
      </div>
      ...
      <Section title="প্রোগ্রাম-ভিত্তিক Registration">
        {data.projects.length === 0 ? (
          <Empty text="এখনো কোনো Event নেই।" />
        ) : (
          <Table
            head={["Program", "গতকাল", "আজ", "মোট"]}
            rows={data.projects.map((p) => [p.project_name, p.registrations_yesterday, p.registrations_today, p.registrations_total])}
          />
        )}
      </Section>

      <Section title="Communicator-ভিত্তিক Task Activity">
        {data.executives.length === 0 ? (
          <Empty text="এখনো কোনো Communicator অ্যাকাউন্ট তৈরি হয়নি।" />
        ) : (
          <Table
            head={["Communicator", "গতকাল Complete করেছে", "মোট Complete", "এখনো Pending"]}
...
function TodayTaskStageSummary({ date, stages = [], conversions = [] }) {
  ...
  return (
    <Section title={`${date || ""} — Communicator Task Stage Summary`}>
      <div className="mb-4 flex items-center justify-between rounded-xl bg-indigo-50 px-4 py-3">
        <span className="text-xs font-bold text-indigo-800">মোট completed contact</span>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <BreakdownList title="Stage conversion" rows={conversions.map((item) => ({ name: `${item.from_stage} → ${item.to_stage}`, count: item.count }))} />
      </div>
    </Section>
  );
}
    ...
    <div className="rounded-xl border border-[#E4E7EC] bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-0.5 text-[11px] text-[#667085]">{data?.date || "—"}</div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="space-y-1.5">
        ...
        )) : <div className="rounded-lg bg-slate-50 px-3 py-4 text-center text-[11px] text-slate-400">কোনো lead নেই</div>}
      </div>
    </div>
  );
}
